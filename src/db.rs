...
impl JiraDatabase {
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        ...
        db_state
            .epics
            .get_mut(&epic_id)
            .ok_or_else(|| anyhow!("Epic {epic_id} not found"))?
            .stories
            .push(story_id);
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        let mut db_state = self.read_db()?;
        match db_state.epics.remove(&epic_id) {
            Some(epic) => epic.stories.iter().for_each(|story_id| {
                db_state.stories.remove(story_id);
            }),
            None => return Err(anyhow!("Epic {epic_id} not found")),
        }
        self.database.write_db(&db_state)?;
        Ok(())
    }
    ...
    pub fn delete_story(&self, epic_id: u32, story_id: u32) -> Result<()> {
        let mut db_state = self.read_db()?;
        match db_state.epics.get_mut(&epic_id) {
            Some(epic) if epic.stories.contains(&story_id) => epic
                .stories
                .remove(epic.stories.iter().position(|&id| id == story_id).unwrap()),
            Some(_) => return Err(anyhow!("Story {story_id} not found in Epic {epic_id}")),
            None => return Err(anyhow!("Epic {epic_id} not found")),
        };
        if db_state.stories.remove(&story_id).is_none() {
            return Err(anyhow!("Story {story_id} not found"));
        }
        self.database.write_db(&db_state)?;
        Ok(())
    }
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<()> {
        let mut db_state = self.read_db()?;
        match db_state.stories.get_mut(&story_id) {
            Some(story) => story.status = status,
            None => return Err(anyhow!("Story {story_id} not found")),
        }
        self.database.write_db(&db_state)?;
        Ok(())
    }
