    ...
    pub fn handle_action(&mut self, action: Action) -> Result<()> {
        match action {
            ...
            Action::UpdateEpicStatus { epic_id } => {
                if let Some(updated_status) = (self.prompts.update_status)() {
                    self.db
                        .update_epic_status(epic_id, updated_status)
            ...
            Action::CreateStory { epic_id } => {
                let created_story = (self.prompts.create_story)();
                self.db
                    .create_story(created_story, epic_id)
                    .with_context(|| anyhow!("failed to create story!"))?;
            }
            Action::UpdateStoryStatus { story_id } => {
                if let Some(updated_status) = (self.prompts.update_status)() {
                    self.db
                        .update_story_status(story_id, updated_status)
